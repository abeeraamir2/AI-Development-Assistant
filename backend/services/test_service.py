...
def generate_tests(requirement_text, strategy_dict):
    """
    FIX: previously called `model.generate_content(...)` with
    `genai.GenerationConfig(...)` — neither `model` nor `genai` were
    defined/imported in this file (only `client` and `MODEL_NAME` were).
    That was leftover code from the OLD `google.generativeai` SDK style.
    This project uses the NEW `google.genai` SDK everywhere else (see
    analysis_service.py) — client.models.generate_content(...) with
    types.GenerateContentConfig(...). Also removed the duplicate copy of
    this exact function that was defined twice further down.
    """
